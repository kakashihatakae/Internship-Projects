...
viewid = 1;
vset = addView(vset, viewid, 'Points', p2, 'Orientation',R2, 'Location' ,T2);
...
viewid = viewid + 1;
vset = addView(vset, viewid, 'Points',p3, 'Orientation',orient, 'Location',loc);
vset = addConnection(vset, viewid-1, viewid, 'Matches', index2_3);
...
%     locations(:,2), 'ZData', locations(:,3)); 
    ...
    if mod(viewid, 7) == 0
       ...
       [xyz, reproje] = triangulateMultiview(tracks, camposes, cameraMat);
       fixedIds = [startframe, startframe+1];
       
       idx = reproje <2;
       
       [~, camPoses] = bundleAdjustment(xyz(idx, :), tracks(idx), ...
